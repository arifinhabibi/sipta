...
export default function Page() {
  ...
  // Fungsi untuk menghitung total point dan grade
  const calculateTotalGrade = (
    studentId: string
  ): { total: number; grade: string } => {
    const studentAccomplishments = accomplishments[studentId] || {};
    const total = Object.values(studentAccomplishments).reduce(
      (sum, score) => sum + score,
      0
    );
    const grade = generateGrade(total);
    return { total, grade };
  };
  ...
  useEffect(() => {
    ...
    // Tambah 15 menit ke waktu mulai
    const limitTime = new Date(startTime.getTime() + 15 * 60 * 1000);

    // Jika sekarang sudah lewat dari 15 menit setelah start_time
    if (now > limitTime) {
      toast.error(
        "Akses ditolak: sudah melewati 15 menit dari waktu mulai pelajaran."
      );
  ...
  useEffect(() => {
    if (schedule?.classroom?.students) {
      ...
      const initAccomplishments: Record<string, Record<string, number>> = {};
      ...
      schedule.classroom.students.forEach((student: any) => {
        ...
        student.accomplishments?.forEach((acc: any) => {
          // Jika ada nilai sebelumnya, gunakan nilai tersebut, else default 0
          accMap[acc.accomplishment_id] = acc.score || 0;
          gradeMap[acc.accomplishment_id] = generateGrade(acc.score || 0);
        });
  ...
  const handleSubmit = () => {
    ...
    const payload = schedule?.classroom.students.map((student: Student) => ({
      ...
      accomplishments: schedule.accomplishments.map((acc: any) => ({
        ...
        score: accomplishments[student.id]?.[acc.id] || 0,
        grade: grades[student.id]?.[acc.id] || "E",
      })),
    }));
                          ...
                          <div className="mt-3">
                            ...
                            <div className="space-y-3">
                              {schedule?.accomplishments?.map((acc: any) => {
                                const currentScore =
                                  accomplishments[student.id]?.[acc.id] || 0;
                                const currentGrade =
                                  grades[student.id]?.[acc.id] || "E";
                                const gradeColor =
                                  {
                                    A: "text-green-600",
                                    B: "text-blue-600",

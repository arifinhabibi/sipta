...
export default function Page() {
  ...
  useEffect(() => {
    if (schedule?.classroom?.students) {
      ...
      const initAccomplishments: Record<string, Record<string, boolean>> = {};
      ...
      schedule.classroom.students.forEach((student: any) => {
        ...
        student.accomplishments?.forEach((acc: any) => {
          accMap[acc.accomplishment_id] = acc.is_capable;
        });
  ...
  const handleAccomplishmentChange = (studentId: string, accId: string) => {
    setAccomplishments((prev) => ({
      ...prev,
      [studentId]: {
        ...prev[studentId],
        [accId]: !prev[studentId][accId],
      },
    }));
  };
  ...
  const handleSubmit = () => {
    ...
    const payload = schedule?.classroom.students.map((student: Student) => ({
      ...
      accomplishments: schedule.accomplishments.map((acc: any) => ({
        ...
        is_capable: accomplishments[student.id]?.[acc.id] || false,
      })),
    }));
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 pb-20">
        ...
        <main className="max-w-7xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            ...
                            {schedule?.accomplishments?.map((acc: any) => (
                              ...
                              >
                                <input
                                  type="checkbox"
                                  checked={
                                    accomplishments[student.id]?.[acc.id] ||
                                    false
                                  }
                                  onChange={() =>
          ...
          <div className="flex justify-between px-4 py-3 border-t bg-white sticky bottom-0">
            <button
              onClick={saveDraft}
              className="flex-1 py-2 mr-2 border rounded-lg text-gray-700 font-medium"
            >
              Simpan Draft
            </button>
            <button
              onClick={handleSubmit}
              className="flex-1 py-2 ml-2 bg-blue-500 text-white font-medium rounded-lg"
            >
              Submit
            </button>
          </div>
